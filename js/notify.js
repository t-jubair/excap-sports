/* ============================================================
   ...
   (function () {
    ...
    /* Brand kit invitation — different copy per registration type.
       Included only in the approval email, professional + personal. */
    function brandKitMessage(regType) {
      ...
      const copy = {
        team: {
          ...
          body: "Planning a team jersey, banner, or match-day reel? Grab the official EX-CAP logos, cover photos and posters from the brand kit. Feel free to share it on your timeline — help build the pre-match vibe and let everyone know your squad is in.",
          cta: "🎨 Open Brand Kit"
        },
        guest: {
      ...
      return `<div style="margin-top:22px;background:linear-gradient(135deg,rgba(124,58,237,.08),rgba(219,39,119,.06));border:1px solid rgba(124,58,237,.25);border-radius:14px;padding:20px 22px">
        ...
        <a href="${e(brandUrl)}" style="display:inline-block;background:linear-gradient(120deg,#7c3aed,#db2777);color:#ffffff;text-decoration:none;padding:10px 18px;border-radius:10px;font-weight:800;font-size:13px;letter-spacing:.03em">${c.cta} →</a>
      </div>`;
    }
    ...
    Notify.sendApprovalEmail = function ({ toEmail, toName, regId, regType, venue, eventDate, message, subject, rec }) {
      ...
      const brandKitBlock = brandKitMessage(regType);
